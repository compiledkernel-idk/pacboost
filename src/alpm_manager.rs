...
use alpm::{Alpm, LogLevel, SigLevel};
...
impl AlpmManager {
    pub fn new() -> Result<Self> {
        ...
        let mut handle =
            Alpm::new(root, dbpath).map_err(|e| anyhow!("failed to initialize libalpm: {}", e))?;

        // Set Cache Directory
        handle
            .add_cachedir("/var/cache/pacman/pkg/")
            .map_err(|e| anyhow!("failed to set cachedir: {}", e))?;
        ...
        handle.set_question_cb((), |any_question, _| {
            use alpm::Question;
            if let Question::Replace(q) = any_question.question() {
                eprintln!(
                    ":: replacing {} with {}...",
                    q.oldpkg().name(),
                    q.newpkg().name()
    ...
    fn parse_pacman_conf() -> Result<Vec<(String, Vec<String>)>> {
        ...
        if repos.is_empty() {
            repos = vec![
                ...
                (
                    "extra".to_string(),
                    vec!["https://geo.mirror.pkgbuild.com/extra/os/x86_64".to_string()],
                ),
                (
                    "multilib".to_string(),
                    vec!["https://geo.mirror.pkgbuild.com/multilib/os/x86_64".to_string()],
                ),
            ];
        }
    ...
    pub async fn sync_dbs_manual(
        ...
        for db in self.handle.syncdbs() {
            ...
            if !servers.is_empty() {
                tasks.push(crate::downloader::TurboTask::new(
                    servers,
                    format!("{}.db", db.name()),
                ));
            } else {
