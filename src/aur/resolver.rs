...
use super::client::{AurClient, AurPackageInfo, parse_dependency};
...
impl DependencyGraph {
    ...
    fn add_official_node(&mut self, name: &str, repo: &str) {
        self.nodes.insert(name.to_string(), PackageNode {
            name: name.to_string(),
            version: String::new(),
            source: PackageSource::Official { repo: repo.to_string() },
            info: None,
        });
    }
    ...
    fn add_aur_node(&mut self, info: AurPackageInfo) {
        self.nodes.insert(info.name.clone(), PackageNode {
    ...
    fn visit_node(
        ...
        if temp_mark.contains(node) {
            ...
            let cycle_start = cycle_path.iter()
                .position(|n| n == node)
                .unwrap_or(0);
...
mod tests {
    ...
    fn test_topological_sort_simple() {
        ...
        graph.nodes.insert("a".to_string(), PackageNode {
            name: "a".to_string(),
            version: "1.0".to_string(),
            source: PackageSource::Aur { votes: 0, popularity: 0.0 },
            info: None,
        });
        graph.nodes.insert("b".to_string(), PackageNode {
            name: "b".to_string(),
            version: "1.0".to_string(),
            source: PackageSource::Aur { votes: 0, popularity: 0.0 },
            info: None,
        });
        graph.nodes.insert("c".to_string(), PackageNode {
            name: "c".to_string(),
            version: "1.0".to_string(),
            source: PackageSource::Aur { votes: 0, popularity: 0.0 },
            info: None,
    ...
    fn test_circular_dependency_detection() {
        ...
        // A -> B -> C -> A (circular)
        for name in &["a", "b", "c"] {
            graph.nodes.insert(name.to_string(), PackageNode {
                name: name.to_string(),
                version: "1.0".to_string(),
                source: PackageSource::Aur { votes: 0, popularity: 0.0 },
                info: None,
