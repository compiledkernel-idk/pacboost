...
use super::pkgbuild::{SecurityReport, SecurityValidator};
...
impl AurBuilder {
    ...
    pub async fn fetch(&self, info: &AurPackageInfo) -> Result<PathBuf> {
        ...
        if extract_dir.exists() {
            ...
            fs::remove_dir_all(&extract_dir).context("Failed to clean old build directory")?;
        }
    ...
    pub fn import_pgp_keys(&self, build_dir: &Path) -> Result<()> {
        ...
        for key in &keys {
            ...
            if !imported {
                println!(
                    "      {} could not import key {} (will try --skippgpcheck)",
                    style("->").yellow(),
                    key
                );
            }
    ...
    pub fn build(&self, build_dir: &Path, package_name: &str) -> Result<Vec<PathBuf>> {
        ...
        if report.issues.is_empty() {
            ...
            println!(
                "   {} security scan complete (score: {}/100)",
                style("->").yellow(),
                report.score
            );
            ...
            if report.score < self.min_security_score {
                return Err(PacboostError::PkgbuildSecurityIssue {
                    reason: format!(
                        ...
                        report.score, self.min_security_score
                    ),
        ...
        let pkgext = if self.disable_compression {
            ".pkg.tar"
        } else {
            ".pkg.tar.zst"
        };
    ...
    pub fn install(&self, packages: &[PathBuf]) -> Result<()> {
        ...
        let package_args: Vec<&str> = packages.iter().filter_map(|p| p.to_str()).collect();
...
pub async fn install_aur_packages_with_deps(
    ...
    println!(
        "{} Dependency resolution completed in {:.2}s",
        style("::").cyan().bold(),
    ...
    if !conflicts.is_empty() {
        ...
        println!(
            "{} {}",
            style("::").yellow().bold(),
            style("Conflicts detected:").yellow().bold()
        );
    ...
    println!(
        "{} {}",
        style("::").cyan().bold(),
        style("Package Summary").white().bold()
    );
    println!();
    ...
    for pkg in &sorted_aur {
        ...
        table.add_row(vec![
            ...
            Cell::new(format!("{}", pkg.num_votes)).fg(if pkg.num_votes > 100 {
                Color::Green
            ...
            Cell::new(format!("{:.1}", pkg.popularity)).fg(if pkg.popularity > 1.0 {
                Color::Green
            } else {
                Color::Yellow
            }),
            Cell::new(maintainer).fg(if maintainer == "orphan" {
                Color::Red
            } else {
                Color::White
            }),
            Cell::new(ood).fg(if ood == "Yes" {
                Color::Red
            } else {
                Color::Green
            }),
    ...
    println!(
        "{} {}",
        style("::").cyan().bold(),
        style("Package Details").white().bold()
    );
    println!();
    ...
    for pkg in &sorted_aur {
        ...
        if !pkg.make_depends.is_empty() {
            println!(
                "      Make Deps: {}",
                style(pkg.make_depends.join(", ")).dim()
            );
        }
    ...
    println!(
        "{} {}",
        style("::").cyan().bold(),
        style("Build Configuration").white().bold()
    );
    println!(
        "   Packages to build: {}",
        style(sorted_aur.len()).yellow().bold()
    );
    println!("   Total dependencies: {}", style(total_deps).white());
    println!(
        "   Total make dependencies: {}",
        style(total_makedeps).white()
    );
    println!(
        "   Parallel jobs: {}",
        style(config.get_make_jobs()).green()
    );
    println!(
        "   Compression: {}",
        style(if config.aur.disable_compression {
            "disabled (fast)"
        } else {
            "enabled"
        })
    ...
    println!();
    print!(
        "{} Proceed with build and installation? [Y/n] ",
        style("::").cyan().bold()
    );
    ...
    println!(
        "{} {}",
        style("::").green().bold(),
        style("Starting build process...").white().bold()
    );
    ...
    println!(
        "{} {}",
        style("::").green().bold(),
        style("Installation complete").white().bold()
    );
