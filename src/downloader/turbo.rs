...
use tokio::io::{AsyncSeekExt, AsyncWriteExt, AsyncReadExt, BufWriter};
use tokio::sync::{Semaphore, RwLock, Mutex, mpsc};
...
async fn download_turbo_with_retry(
    ...
    Err(last_error.unwrap_or_else(|| anyhow!("download failed after {} retries", config.max_retries)))
}
...
async fn download_turbo(
    ...
    let sorted_mirrors = {
        ...
        let mut mirrors: Vec<_> = task.mirrors.iter()
            .map(|m| {
    ...
    let num_segments = if num_segments > 1 && file_size / num_segments as u64 >= config.min_segment_size {
        num_segments
    } else {
...
async fn probe_racing(
    ...
    for mirror in mirrors.iter().skip(race_count) {
        if let Ok(Some((size, supports_ranges))) = timeout(
...
async fn download_segmented(
    ...
    let incomplete: Vec<_> = segments.iter()
        .enumerate()
...
async fn download_segment_with_retry(
    ...
    Err(anyhow!("segment download failed after {} retries", config.max_retries))
}
                        ...
                        match chunk_result {
                            Ok(Some(Ok(data))) => {
                                ...
                                if data.len() > MAX_CHUNK_SIZE {
                                    last_error = Some(format!("chunk too large: {} bytes", data.len()));
                                    break;
                                }
